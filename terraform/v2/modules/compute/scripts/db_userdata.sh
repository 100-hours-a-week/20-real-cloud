...
# 4. 비밀번호 SSM에서 가져오기
MYSQL_ROOT_PASSWORD=$(aws ssm get-parameter --name "/prod/mysql/root_password" --with-decryption --query "Parameter.Value" --output text)
...
BUCKET="s3://your-bucket-name/mysql-backups"
...
BUCKET="s3://your-bucket-name/mysql-logs"
DATE=$(date +"%Y-%m-%d_%H-%M-%S")

aws s3 cp $LOG_SRC "$BUCKET/mysqld-$DATE.log"
EOS
chmod +x $LOG_SCRIPT
