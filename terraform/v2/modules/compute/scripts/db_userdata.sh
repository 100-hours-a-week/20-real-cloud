...
DB_PASSWORD=$(aws ssm get-parameter --region ap-northeast-2 --name "/config/prod/db_passwd" --with-decryption --query "Parameter.Value" --output text)
...
mkdir -p $TMP_DIR
LATEST_DUMP=$(aws s3 ls --region ap-northeast-2 $S3_BUCKET/ | sort | tail -n 1 | awk '{print $4}')
aws s3 cp --region ap-northeast-2 "$S3_BUCKET/$LATEST_DUMP" "$TMP_DIR/latest_dump.sql.gz"
...
gzip "$DUMP_FILE"
aws s3 cp --region ap-northeast-2 "$GZ_FILE" "$BUCKET/"
rm -f "$GZ_FILE"
EOS
chmod +x $CRON_SCRIPT
